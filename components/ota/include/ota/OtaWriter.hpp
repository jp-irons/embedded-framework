 ...
 * HttpRequest::receiveChunk() — no large heap allocation required.  On
 * success, sets the newly-written partition as next-boot and calls
 * esp_restart().
 *
...
class OtaWriter {
  public:
    ...
    /**
     * Perform the OTA update by streaming from the request body.
     *
     ...
     */
    static bool writeFromRequest(http::HttpRequest &req, http::HttpResponse &res);
};
